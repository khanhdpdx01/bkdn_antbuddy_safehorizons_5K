...
class CartRepository extends BaseRepository {
    ...
    async findByCartId(cartId, columns = '*') {
        ...
        lineItems = await Promise.all(lineItems.map(async (lineItem) => {
            const product = await knex('products').select(columns)
                .where({ 'products.product_id': lineItem.product_id }).first();
            const supplier = await knex('suppliers').select(columns)
            ...
            subTotalPrice += lineItem.quantity * lineItem.price
    ...
    async findCartItems(cartId, productId, columns = ['*']) {
        const cartItem = await knex('cart_items').select(columns)
